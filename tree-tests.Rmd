...
#Replace species names in tree with those in table:
rep.table<-na.omit(data.frame(add.sp, to.graft))

for(i in 1:length(ptree$tip.label)){
  for(j in 1:length(rep.table$add.sp)){
    if(ptree$tip.label[i]==rep.table$add.sp[j]){
      ptree$tip.label[i]<-rep.table$to.graft[j]
    }
  }
}

ptree<-as.phylo(ptree)
write.tree(ptree, file="tree_298.tre")
