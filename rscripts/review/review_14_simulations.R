...
# read imputed data
df_imputed <- read.csv("outputs/imputed_without_phylo.csv", row.names = 1)
continuous_traits <- df_imputed[,c(1:7)]

# make discrete traits numeric
discrete_traits <- df_imputed[,c(8:21)]
discrete_traits <- as.data.frame(lapply(discrete_traits, as.factor))
discrete_traits <- as.data.frame(lapply(discrete_traits, as.numeric))

# make new imputed data frame
df_imputed <- cbind(continuous_traits,discrete_traits)
...
ou_list <- lapply(continuous_traits,fit_cont)
saveRDS(ou_list,"outputs/14_non-phylo_ou_list.rds")
...
fit_disc <- function(ttf) {
  ...
  fit <- castor::asr_mk_model(angio_tree,as.numeric(ttf),rate_model = "ARD")
...
saveRDS(mk_list,"outputs/14_non-phylo_mk_list.rds")
...
# NOTE: Can comment if already run
ou_list <- readRDS("outputs/14_non-phylo_ou_list.rds")
mk_list <- readRDS("outputs/14_non-phylo_mk_list.rds")
n_sim <- 1000
set.seed(123)
simdata <- replicate(n_sim,simul_dataset(ou_list,mk_list,angio_tree),simplify = F)
saveRDS(simdata,"outputs/14_non-phylo_phylo_simulated_datasets.rds")

####
## Figure S6: Simulation eigenvalues ----
####

# read in previously simulated data sets
sim_list <- readRDS("outputs/14_non-phylo_phylo_simulated_datasets.rds")
...
# Compute simulated eigenvalues
# NOTE: Can comment if already run
sim_ev <- sapply(sim_list,pc_ev)
saveRDS(data.frame(t(sim_ev)),"outputs/14_non-phylo_phylo_simulated_ev.rds")

# read in previously calculated eigenvalues
sim_ev <- readRDS("outputs/14_non-phylo_phylo_simulated_ev.rds")
...
onehot_ev <- eo[c(1:21)]
er <- pcoa(daisy(df_imputed,"gower"))$values$Relative_eig
er <- er[er>0]/sum(er[er>0])
imputed_ev <- er[c(1:21)]
rm(ef,eo,er)
df_ev <- data.frame(
  list(
...
ggplot(data = df_ev,aes(x=c(1:21))) +
  ...
  #  geom_line(aes(y=imputed,col="imputed")) + # not needed
  #  geom_point(aes(y=imputed,col="imputed")) +
...
ggsave("figures/figure_S6_eigenvalues_simulations_non-phylo.png",width=10,height=10)
...
# ggsave("figures/14_non-phylo_pcoa_sim_ex1.png")
...
ggarrange(plotlist = list(g1,g2),ncol = 1,nrow = 2)
# ggsave("figures/14_non-phylo_pcoa_sim_ex2.png")
...
ggarrange(plotlist = list(g1,g2),ncol = 1,nrow = 2)
# ggsave("figures/14_non-phylo_pcoa_sim_ex3.png")
...
png("figures/figure_S2_scatterplots_pairwise_distances_non-phylo.png",width=1500,height=1500,res=100)
