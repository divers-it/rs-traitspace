...
png("figures/agglomerative_hclust_ward_dendrogram.png",width=3000,height=3000,res=300)
...
# Agglomerative ward
ggplot(data = data.frame(t(cstats.table(gower_df, aggl.clust.w, 10))), 
       aes(x=cluster.number, y=within.cluster.ss)) + 
  geom_point()+
  geom_line()+
...
wk3 + wk6
ggsave("figures/dendro_ward_k3_k6.png", width = 20, height= 10)
...
clust.long <- melt(data.frame(lapply(df2.cl, as.character), stringsAsFactors=FALSE), 
...
heatmap.p
ggsave("figures/hclust_characteristics_qual.png",width=10,height=15)
...
(c1 + c2) / (c3 + c4) / (c5 + c6)
ggsave("figures/hclust_characteristics_quant.png",width = 15,height=20)
...
ggsave("figures/pcoa_hclust_k3.png",width=12,height=10)
...
ggsave("figures/pcoa_hclust_k6.png",width=12,height=10)
