...
df_dclust$center[dataset_single$center[i]]=df_dclust$Row.names[dataset_single$center[i]]
}
...
#    shape = 21,
   ...
   geom_text_repel(aes(label = df_dclust$center, colour = as.factor(df_dclust$cluster)), size = 3.5) + 
#							 stat_ellipse(geom = "polygon",
#                                                   aes(fill = as.factor(clust.num)),
#                                                   alpha = 0.25) +
  xlab(paste(
    "Axis 1: relative eigenvalue =",
    round(dataset_pcoa$values$Relative_eig[1]/sum(dataset_pcoa$values$Relative_eig[dataset_pcoa$values$Relative_eig>0]), 2)
  )) +
  ylab(paste(
