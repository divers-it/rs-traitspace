...
saveRDS(df_recoded,"outputs/imputed_recoded.RDS")
...
# Compare the different distances
pdf("figures/pcoa/distance_comparison.pdf")
pairs(
  data.frame(list(
    "final"=as.vector(dist_final),
    "onehot"=as.vector(dist_onehot),
    "recoded"=as.vector(dist_recoded))
  ),pch=16,cex=0.5
)
dev.off()
...
# Mapping the traits on the pcoa ####
...
# A their too many plots only the final distance matrix is used
# But onehot and recoded matrices can be also run by encommenting the following code
