...
A preprint of this study can be found [here](https://doi.org/10.1101/2024.02.26.582019)
...
Note that there are two encodings that are used for these data. The original encoding has traits with as many values as there are states (e.g. woody, herbaceous, woody_herbaceous). For the one-hot encoding, each state is recoded to a binary trait (e.g. trait 1: woody, non-woody | trait 2: herbaceous, non-herbaceous). Scripts 1, 3, 4, 5 and 6 have corresponding one-hot encoding versions with the suffix "one_hot".
...
The dataset is then filtered, removing traits with more than 60% missing data and species with more than 50% missing data. Outliers for each of the traits are also removed and the data is saved as an RDS. Species names are also checked and corrected in this script.
...
Examines the correlation between traits in the data set and clusters traits into groups (Fig. 3b).

![**Figure 2**. (a) One-hot traitspace with eigenvectors and (b) trait correlation matrix with dendrogram.](figures/figure_2_loadings.png){width=800px}
...
![**Figure 4.** Scatterplot of PCOA where each point indicates a species.](./figures/figure_4_pcoa.png){width=800px}
...
![**Figure S3**. Diaz et al. 2022 comparison.](figures/figure_S3_Diaz_comparison.png){width=600px}
...
![**Figure S5**. Influence of number of dimensions on the quality of trait space.](figures/figure_S5_dimensionality.png){width=800px}

#### 10_clustering_PAM.R

The [Partitioning Around Medoids](https://dpmartin42.github.io/posts/r/cluster-mixed-types) (PAM) approach is used. All species must be assigned to a cluster and some species may be spuriously assigned to clusters. Examples of cluster membership when number of clusters (k) is 2 and 3 is shown in Figure 4a.

![**Figure 3**. Panel (a) shows PCoA with clustering where colours (k = 2) and shapes (k = 3) correspond to PAM clusters. Panel (b) shows UMAP plot see below) where shapes correspond to k = 3 clusters.](figures/figure_3_umap.png){width=800px}

To avoid interpreting species than are not well-clustering we look at 'robust groups' - those species that consistently stick together as the value of k is changed. A Sankey plot demonstrates how species move between clusters as k is increased. 

![**Figure S11**. A Sankey plot representing how species are shared among clusters when values of k are increased from 2-7.](figures/figure_S11_sankey_PAM.png){width=600px}

We can then visualize how these robust groups relate to clusters, and also how traits differ among robust groups.

![**Figure S12**. Robust groups from PAM clustering and corresponding state frequences / trait values.](figures/figure_S12_robust_groups.png){width=800px}

#### 11_phylo_plot

Plots a phylogenetic tree of all 360 species in our data set with trait data at the tips. 

![**Figure 1**. Phylogenetic tree.](figures/figure_1_phylo.png){width=800px}

#### 12_loadings.R

Builds a traitspace using one-hot encoded data. Eigenvectors are plotted on the first two axes, similar to 'loadings' from a PCA biplot, as shown in Figure 3a. 

![**Figure 2**. (a) One-hot traitspace with eigenvectors and (b) trait correlation matrix with dendrogram.](figures/figure_2_loadings.png){width=800px}

We can also examine other axes to see which traits vary along them.

![**Figure S7**. One-hot traitspace with eigenvectors for axes 3 and 4.](figures/figure_S7_loadings_axes_3_4.png){width=600px}

#### 13_UMAP.R

Performs Uniform Manifold Approximation and Projection (UMAP), a non-linear dimensionality reduction approach. 

![**Figure S10**. UMAP trait spaces with different values of neighbourhood size.](figures/figure_S10_scatterplots_umap_knn10-100.png){width=600px}
...
![**Figure S6**. Comparison of simulated (black error bars) versus empirical (coloured lines) eigenvalues.](figures/figure_S6_eigenvalues_simulations.png){width=600px}
...
Contains scripts that are not used in the current version of the manuscript (but could be useful in the future).
